...
import { Calculator, Shirt, RefreshCcw, Package, FileText, List, X, CheckCircle, Clock, XCircle, ChevronDown, ChevronUp, ClipboardList, Search, Plus, Trash2 } from "lucide-react";
...
const cardStyle = {
  background: TM.white, borderRadius: 16, padding: 28,
...
const inputStyle = {
  ...
  boxSizing:"border-box", background:TM.white, color:TM.ink, fontFamily:"inherit",
};
...
const buttonStyle = {
  ...
  background:TM.white, color:TM.ink, cursor:"pointer", fontWeight:600, fontSize:13, fontFamily:"inherit",
};
...
function newLineItem(id) {
  return {
    ...
    oneOffName: "",
    oneOffCost: "",
    oneOffSizes: { XS:0,S:0,M:0,L:0,XL:0,"2XL":0,"3XL":0,"4XL":0 },
    // Decoration
    frontPrint: "1",
    backPrint: "none",
    hasSleevePrint: false,
    frontPrintCost: 2,
    backPrintCost: 4,
    frontBackComboCost: 6,
    sleevePrintCost: 1.4,
  };
}
...
function SummaryRow({label,value,bold=false,accent=false}) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",padding:"7px 0",fontSize:13,
      borderTop:bold?`1px solid ${TM.border}`:"none",marginTop:bold?6:0,fontWeight:bold?800:500}}>
...
function StageBadge({stage}) {
  const c={
    "Order Created":{color:"#0369a1",bg:"#e0f2fe"},"Materials Ordered":{color:"#7c3aed",bg:"#ede9fe"},
    "In Production":{color:"#92400e",bg:"#fef3c7"},"Quality Check":{color:TM.tanDeep,bg:TM.tanBg},
    "Ready":{color:TM.success,bg:"#d1fae5"},"Pickup / Shipped / Delivered":{color:TM.dark,bg:TM.mutedBg},
    "Completed":{color:TM.success,bg:"#d1fae5"},"Cancelled":{color:TM.danger,bg:"#fee2e2"},
...
function SanMarSearch({ onSelect }) {
  ...
  const search = useCallback(async (q) => {
    ...
    const { data, error } = await supabase
      ...
      .order("style").limit(100);
    if (!error && data) { setResults(data); setShowResults(true); }
    setLoading(false);
  }, []);
    ...
    <div style={{ position: "relative" }}>
      <div style={{ position: "relative" }}>
        ...
        <input style={{ ...inputStyle, paddingLeft: 36 }}
          placeholder="Search by style # (PC61) or keyword…"
      ...
      {showResults && results.length > 0 && (
        <div style={{ position:"absolute", zIndex:999, top:"100%", left:0, right:0, background:TM.white, border:`1px solid ${TM.border}`, borderRadius:10, boxShadow:"0 8px 32px rgba(44,42,38,0.15)", maxHeight:280, overflowY:"auto", marginTop:4 }}>
          {results.map(r => (
            <div key={r.style}
...
function SanMarColorSizePicker({ selectedStyle, onColorSelect, onSizeQtyChange, sizeQtys }) {
  ...
  useEffect(() => {
    ...
    setLoadingColors(true); setSelectedColor(""); setSizePrices({});
    supabase.from("sanmar_products").select("color_name").eq("style", selectedStyle)
      .in("product_status", ["Regular","Active","New"]).order("color_name")
      .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    supabase.from("sanmar_products").select("size, piece_price, size_index")
      .eq("style", selectedStyle).eq("color_name", selectedColor)
                ...
                {sortedSizes.map(size => (
                  <div key={size} style={{ background:TM.tanBg, borderRadius:8, padding:"8px 10px", border:`1px solid ${TM.tanBorder}` }}>
                    ...
                    <input type="number" min="0"
                      ...
                      value={sizeQtys[size] || ""} placeholder="0"
...
// ── Line Item Card ────────────────────────────────────────────────────────────
function LineItemCard({ item, index, total, onChange, onRemove }) {
  const updateItem = (patch) => onChange({ ...item, ...patch });

  const allSizeEntries = item.isOneOff
    ? Object.entries(item.oneOffSizes).filter(([,v])=>safeNum(v)>0)
    : Object.entries(item.sizeQtys).filter(([,v])=>safeNum(v)>0);
  const itemQty = allSizeEntries.reduce((s,[,v])=>s+safeNum(v),0);

  const garmentCostEach = useMemo(() => {
    if (item.isOneOff) return safeNum(item.oneOffCost);
    if (!Object.keys(item.sizePrices).length || itemQty === 0) return 0;
    const weighted = allSizeEntries.reduce((s,[sz,qty]) => s + (item.sizePrices[sz]||0)*safeNum(qty), 0);
    return round2(weighted / itemQty);
  }, [item.isOneOff, item.oneOffCost, item.sizePrices, item.sizeQtys, itemQty]);

  const decorationCostEach = (() => {
    const hasFront = item.frontPrint !== "none"; const hasBack = item.backPrint !== "none";
    let d = 0;
    if (hasFront && hasBack) d = safeNum(item.frontBackComboCost);
    else if (hasFront) d = safeNum(item.frontPrintCost);
    else if (hasBack) d = safeNum(item.backPrintCost);
    if (item.hasSleevePrint) d += safeNum(item.sleevePrintCost);
    return round2(d);
  })();

  const garmentLabel = item.isOneOff
    ? (item.oneOffName || "Custom Item")
    : item.selectedStyle
      ? `${item.selectedStyle.style} — ${item.selectedStyle.product_title}${item.selectedColor ? ` (${item.selectedColor})` : ""}`
      : "No garment selected";

  // Expose calculated values up to parent
  item._garmentCostEach = garmentCostEach;
  item._decorationCostEach = decorationCostEach;
  item._qty = itemQty;
  item._garmentLabel = garmentLabel;

  return (
    <div style={{ background:TM.tanBg, borderRadius:12, padding:18, border:`1px solid ${TM.tanBorder}`, marginBottom:12 }}>
      {/* Item header */}
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:24, height:24, borderRadius:6, background:TM.dark, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:800, color:TM.tan }}>{index+1}</div>
          <span style={{ fontSize:13, fontWeight:700, color:TM.dark }}>
            {garmentLabel !== "No garment selected" ? garmentLabel : `Item ${index+1}`}
          </span>
          {itemQty > 0 && <span style={{ fontSize:12, color:TM.muted }}>· {itemQty} pcs</span>}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, fontWeight:700, cursor:"pointer", color:item.isOneOff?TM.dark:TM.muted, fontFamily:"Arial, sans-serif", userSelect:"none" }}>
            <div style={{ width:32, height:18, borderRadius:99, background:item.isOneOff?TM.dark:TM.border, position:"relative", cursor:"pointer", flexShrink:0 }}
              onClick={() => updateItem({ isOneOff:!item.isOneOff, selectedStyle:null, sizeQtys:{}, sizePrices:{} })}>
              <div style={{ width:14, height:14, borderRadius:"50%", background:item.isOneOff?TM.tan:TM.white, position:"absolute", top:2, left:item.isOneOff?16:2, transition:"left 0.15s" }}/>
            </div>
            One-off
          </label>
          {total > 1 && (
            <button onClick={onRemove} style={{ ...buttonStyle, padding:"4px 8px", color:TM.danger, borderColor:"#fca5a5", fontSize:12 }}>
              <Trash2 size={13}/>
            </button>
          )}
        </div>
      ...
      {item.isOneOff ? (
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:12, marginBottom:12 }}>
            <Field label="Item Name"><input style={inputStyle} value={item.oneOffName} onChange={e=>updateItem({oneOffName:e.target.value})} placeholder="e.g. Comfort Colors 1717"/></Field>
            <Field label="Cost / piece"><input style={inputStyle} type="number" step="0.01" value={item.oneOffCost} onChange={e=>updateItem({oneOffCost:e.target.value})} placeholder="0.00"/></Field>
          </div>
          <div style={{ fontSize:11, fontWeight:700, color:TM.muted, marginBottom:8, textTransform:"uppercase", letterSpacing:"0.06em" }}>Size Breakdown</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(8,1fr)", gap:8 }}>
            {Object.keys(item.oneOffSizes).map(size => (
              <Field key={size} label={size}>
                <input style={inputStyle} type="number" min="0" value={item.oneOffSizes[size]||""} placeholder="0"
                  onChange={e=>updateItem({oneOffSizes:{...item.oneOffSizes,[size]:e.target.value}})}/>
              </Field>
            ))}
          </div>
        </div>
        ...
        <div>
          <Field label="Search SanMar Products">
            <SanMarSearch onSelect={s => updateItem({ selectedStyle:s, selectedColor:"", sizeQtys:{}, sizePrices:{} })}/>
          </Field>
          {item.selectedStyle && (
            <div style={{ marginTop:8, padding:"8px 12px", background:TM.white, borderRadius:8, border:`1px solid ${TM.tanBorder}`, fontSize:13 }}>
              <span style={{ fontWeight:800, color:TM.dark, marginRight:8, fontFamily:"monospace" }}>{item.selectedStyle.style}</span>
              <span style={{ color:TM.ink }}>{item.selectedStyle.product_title}</span>
              <span style={{ color:TM.muted, marginLeft:6 }}>· {item.selectedStyle.brand}</span>
            </div>
          )}
          <SanMarColorSizePicker
            selectedStyle={item.selectedStyle?.style}
            onColorSelect={(color, prices) => updateItem({ selectedColor:color, sizePrices:prices, sizeQtys:{} })}
            onSizeQtyChange={(size, qty) => updateItem({ sizeQtys:{ ...item.sizeQtys, [size]:qty } })}
            sizeQtys={item.sizeQtys}
          />
        </div>
      )}

      {/* Decoration for this item */}
      <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${TM.tanBorder}` }}>
        <div style={{ fontSize:11, fontWeight:700, color:TM.muted, marginBottom:10, textTransform:"uppercase", letterSpacing:"0.06em" }}>Decoration</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:10 }}>
          <Field label="Front Print">
            <select style={inputStyle} value={item.frontPrint} onChange={e=>updateItem({frontPrint:e.target.value})}>
              <option value="none">None</option><option value="1">1 Color</option><option value="full">Full Color</option>
            </select>
          </Field>
          <Field label="Back Print">
            <select style={inputStyle} value={item.backPrint} onChange={e=>updateItem({backPrint:e.target.value})}>
              <option value="none">None</option><option value="1">1 Color</option><option value="full">Full Color</option>
            </select>
          </Field>
          <div style={{ display:"flex", alignItems:"end", paddingBottom:2 }}>
            <label style={{ display:"flex", gap:6, alignItems:"center", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"Arial, sans-serif" }}>
              <input type="checkbox" checked={item.hasSleevePrint} onChange={e=>updateItem({hasSleevePrint:e.target.checked})} style={{ accentColor:TM.dark }}/>
              Sleeve Print
            </label>
          </div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
          <Field label="Front Cost"><input style={inputStyle} type="number" value={item.frontPrintCost} onChange={e=>updateItem({frontPrintCost:e.target.value})}/></Field>
          <Field label="Back Cost"><input style={inputStyle} type="number" value={item.backPrintCost} onChange={e=>updateItem({backPrintCost:e.target.value})}/></Field>
          <Field label="F+B Combo"><input style={inputStyle} type="number" value={item.frontBackComboCost} onChange={e=>updateItem({frontBackComboCost:e.target.value})}/></Field>
          <Field label="Sleeve Cost"><input style={inputStyle} type="number" value={item.sleevePrintCost} onChange={e=>updateItem({sleevePrintCost:e.target.value})}/></Field>
        </div>
      </div>

      {/* Item totals */}
      {itemQty > 0 && (
        <div style={{ marginTop:12, paddingTop:10, borderTop:`1px solid ${TM.tanBorder}`, display:"flex", justifyContent:"space-between", fontSize:12, color:TM.muted, fontFamily:"Arial, sans-serif" }}>
          <span>{itemQty} pcs · Garment avg {currency(garmentCostEach)} · DTF {currency(decorationCostEach)}</span>
          <span style={{ fontWeight:700, color:TM.dark }}>Subtotal cost: {currency((garmentCostEach + decorationCostEach) * itemQty)}</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:TM.dark,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"Georgia, serif"}}>
      <div style={{background:TM.white,borderRadius:20,padding:44,boxShadow:"0 24px 80px rgba(0,0,0,0.4)",border:`3px solid ${TM.tan}`,width:360,textAlign:"center"}}>
        ...
        <input type="password" style={{...inputStyle,marginBottom:12,border:pwError?`1px solid ${TM.danger}`:`1px solid ${TM.border}`}}
        ...
        <button onClick={attempt} style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:TM.dark,color:TM.tan,fontWeight:800,cursor:"pointer",fontSize:14,letterSpacing:"0.05em"}}>SIGN IN</button>
      </div>
    </div>
  );
}
...
function OrdersDrawer({open,onClose,onViewQuote}) {
  ...
  async function deleteOrder(id){if(!window.confirm("Delete this order?"))return;await supabase.from("orders").delete().eq("id",id);if(expandedId===id)setExpandedId(null);fetchOrders();}
  if(!open)return null;
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex",justifyContent:"flex-end"}}>
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} transition={{type:"spring",damping:28,stiffness:260}}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,paddingBottom:16,borderBottom:`2px solid ${TM.tanBorder}`}}>
          <div><div style={{fontSize:11,fontWeight:700,letterSpacing:"0.1em",color:TM.muted,textTransform:"uppercase"}}>TrueMark</div><h2 style={{margin:0,fontSize:22,fontWeight:800,color:TM.dark}}>Active Orders</h2></div>
        ...
        {!loading&&orders.length===0&&<p style={{color:TM.muted}}>No orders yet.</p>}
        {orders.map(o=>{
              ...
              <div onClick={()=>setExpandedId(isOpen?null:o.id)} style={{padding:"14px 16px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",background:isOpen?TM.tanBg:TM.white,borderBottom:isOpen?`1px solid ${TM.border}`:"none"}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
                    <span style={{fontWeight:800,fontSize:13,fontFamily:"monospace",background:TM.dark,color:TM.tan,padding:"1px 8px",borderRadius:4}}>{o.order_number}</span>
                ...
                <div style={{padding:16}}>
                  ...
                  <div style={{marginBottom:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      ...
                      <button onClick={()=>updateStage(o.id,"Cancelled")} style={{...buttonStyle,padding:"4px 12px",fontSize:11,color:TM.danger,borderColor:"#fca5a5"}}>Cancel</button>
                    </div>
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      {ORDER_STAGES.map((stage,idx)=>(
                        <button key={stage} onClick={()=>updateStage(o.id,stage)} style={{...buttonStyle,padding:"8px 14px",fontSize:12,textAlign:"left",background:o.stage===stage?TM.dark:idx<ORDER_STAGES.indexOf(o.stage)?TM.tanBg:TM.white,color:o.stage===stage?TM.tan:idx<ORDER_STAGES.indexOf(o.stage)?TM.muted:TM.dark,borderColor:o.stage===stage?TM.dark:TM.border,display:"flex",alignItems:"center",gap:8}}>
                          <span style={{width:20,height:20,borderRadius:"50%",fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",background:o.stage===stage?TM.tan:TM.border,color:o.stage===stage?TM.dark:TM.muted,flexShrink:0}}>{idx<ORDER_STAGES.indexOf(o.stage)?"✓":idx+1}</span>
                          {stage}
                        </button>
                      ))}
                    </div>
...
function SavedQuotesDrawer({open,onClose,highlightId}) {
  ...
  async function deleteQuote(id){if(!window.confirm("Delete this quote?"))return;await supabase.from("quotes").delete().eq("id",id);if(expandedId===id)setExpandedId(null);fetchQuotes();}
  if(!open)return null;
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex",justifyContent:"flex-end"}}>
      <div onClick={onClose} style={{position:"absolute",inset:0,background:"rgba(26,24,21,0.6)"}}/>
      <motion.div initial={{x:"100%"}} animate={{x:0}} transition={{type:"spring",damping:28,stiffness:260}}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,paddingBottom:16,borderBottom:`2px solid ${TM.tanBorder}`}}>
          <div><div style={{fontSize:11,fontWeight:700,letterSpacing:"0.1em",color:TM.muted,textTransform:"uppercase"}}>TrueMark</div><h2 style={{margin:0,fontSize:22,fontWeight:800,color:TM.dark}}>Saved Quotes</h2></div>
          <button onClick={onClose} style={{...buttonStyle,padding:"6px 10px"}}><X size={18}/></button>
        </div>
...
export default function App() {
  ...
  const [lineItems,setLineItems]=useState([newLineItem(Date.now())]);

  // Shared quote settings
  ...
  // ── Calculations ─────────────────────────────────────────────────────────
  // Force _computed values onto items during render
  lineItems.forEach(item => {
    const entries = item.isOneOff
      ? Object.entries(item.oneOffSizes).filter(([,v])=>safeNum(v)>0)
      : Object.entries(item.sizeQtys).filter(([,v])=>safeNum(v)>0);
    const q = entries.reduce((s,[,v])=>s+safeNum(v),0);
    let gc = 0;
    if (item.isOneOff) gc = safeNum(item.oneOffCost);
    else if (Object.keys(item.sizePrices).length && q > 0) {
      gc = round2(entries.reduce((s,[sz,qty])=>s+(item.sizePrices[sz]||0)*safeNum(qty),0)/q);
    }
    const hasFront = item.frontPrint!=="none"; const hasBack = item.backPrint!=="none";
    let dc = 0;
    if (hasFront&&hasBack) dc=safeNum(item.frontBackComboCost);
    else if (hasFront) dc=safeNum(item.frontPrintCost);
    else if (hasBack) dc=safeNum(item.backPrintCost);
    if (item.hasSleevePrint) dc+=safeNum(item.sleevePrintCost);
    item._qty = q;
    item._garmentCostEach = round2(gc);
    item._decorationCostEach = round2(dc);
    item._garmentLabel = item.isOneOff
      ? (item.oneOffName||"Custom Item")
      : item.selectedStyle
        ? `${item.selectedStyle.style} — ${item.selectedStyle.product_title}${item.selectedColor?` (${item.selectedColor})`:""}`
        : "No garment selected";
  ...
  const totalQty = lineItems.reduce((s,item)=>s+item._qty,0);
  const totalGarmentSubtotal = lineItems.reduce((s,item)=>s+item._garmentCostEach*item._qty,0);
  const totalDecorationSubtotal = lineItems.reduce((s,item)=>s+item._decorationCostEach*item._qty,0);
  const packagingSubtotal = safeNum(packagingFeePerUnit)*totalQty;
  const fixedFees = safeNum(setupFee)+safeNum(artFee)+safeNum(shippingFee)+safeNum(rushFee);
  const hardCost = totalGarmentSubtotal+totalDecorationSubtotal+packagingSubtotal+fixedFees;
  ...
  const rawPrice = totalQty>0?finalTotal/totalQty:0;
  ...
  const displaySubtotal = isManual?pricePerPiece*totalQty:subtotal;
  const displayTotal = isManual?(includeTax?displaySubtotal*(1+safeNum(salesTaxPct)/100):displaySubtotal):finalTotal;

  const garmentLabel = lineItems.map(i=>i._garmentLabel).join(" + ");
  const avgGarmentCost = totalQty>0?round2(totalGarmentSubtotal/totalQty):0;
  const avgDecorationCost = totalQty>0?round2(totalDecorationSubtotal/totalQty):0;

  if(!authed) return <PasswordScreen onAuth={()=>setAuthed(true)}/>;

  const updateLineItem = (id, updated) => setLineItems(prev=>prev.map(i=>i.id===id?updated:i));
  const removeLineItem = (id) => setLineItems(prev=>prev.filter(i=>i.id!==id));
  const addLineItem = () => setLineItems(prev=>[...prev, newLineItem(Date.now())]);

  const resetDefaults=()=>{
    setQuoteName("Spring Promo DTF Quote"); setCustomerName(""); setSalesRep("EJ");
    setLineItems([newLineItem(Date.now())]);
  ...
  const saveQuote=async()=>{
    ...
    const{error}=await supabase.from("quotes").insert([{
      ...
      garment_type:"multi",
      garment_id:lineItems.map(i=>i.selectedStyle?.style||"one-off").join(","),
      garment_label:garmentLabel,
      garment_cost_each:avgGarmentCost,
      qty_xs:0,qty_s:0,qty_m:0,qty_l:0,qty_xl:0,qty_2xl:0,qty_3xl:0,qty_4xl:0,
      total_qty:totalQty,
      front_print:firstItem.frontPrint,back_print:firstItem.backPrint,has_sleeve_print:firstItem.hasSleevePrint,
      decoration_cost_each:avgDecorationCost,
      ...
      garment_subtotal:round2(totalGarmentSubtotal),
      decoration_subtotal:round2(totalDecorationSubtotal),
      size_upcharge_total:0,packaging_subtotal:round2(packagingSubtotal),
      fixed_fees:round2(fixedFees),overhead:round2(overhead),profit:round2(profit),
      subtotal:round2(subtotal),final_total:round2(finalTotal),
  ...
  const generateQuotePdf=()=>{
    ...
    doc.text(`Total quantity: ${totalQty}`,14,56);
    doc.text(`Generated: ${new Date().toLocaleDateString()}`,pageWidth-14,38,{align:"right"});

    let currentY = 64;

    // One table per line item
    lineItems.forEach((item, idx) => {
      if (item._qty === 0) return;
      const displaySizes = item.isOneOff
        ? Object.entries(item.oneOffSizes).filter(([,v])=>safeNum(v)>0)
        : Object.entries(item.sizeQtys).filter(([,v])=>safeNum(v)>0);
      displaySizes.sort(([a],[b])=>(SIZE_ORDER.indexOf(a)===-1?99:SIZE_ORDER.indexOf(a))-(SIZE_ORDER.indexOf(b)===-1?99:SIZE_ORDER.indexOf(b)));

      // Item label
      doc.setFont("helvetica","bold"); doc.setFontSize(10);
      doc.text(`Item ${idx+1}: ${item._garmentLabel}`, 14, currentY);
      currentY += 5;

      autoTable(doc,{
        startY:currentY, theme:"grid",
        head:[["Size",...displaySizes.map(([sz])=>sz)]],
        body:[["Qty",...displaySizes.map(([,v])=>String(safeNum(v)))]],
        styles:{fontSize:9,cellPadding:3,halign:"center"},
        headStyles:{fillColor:[44,42,38],fontStyle:"bold"},
    ...
    autoTable(doc,{
      startY:currentY+4, theme:"grid",
      head:[["Item","Qty","Unit Price","Total"]],
      body:lineItems.filter(i=>i._qty>0).map((item,idx)=>{
        const itemSubtotal = (item._garmentCostEach+item._decorationCostEach)*item._qty;
        const itemPricePerPiece = isManual ? safeNum(manualPriceEach) : (pdfSubtotal/totalQty||0);
        return [`${idx+1}. ${item._garmentLabel}`, item._qty, currency(itemPricePerPiece), currency(itemPricePerPiece*item._qty)];
      }),
      styles:{fontSize:9,cellPadding:3},
      headStyles:{fillColor:[44,42,38],fontStyle:"bold"},
      columnStyles:{0:{cellWidth:90},1:{halign:"center",cellWidth:15},2:{halign:"right",cellWidth:30},3:{halign:"right",cellWidth:30}},
    });
    ...
    autoTable(doc,{
      ...
      body:[
        ...
        ...(includeCcFee?[[`CC Fee ${ccFeePct}%`,"applied",currency(pdfCcFee)]]:[]),
    ...
    const tableEndY=doc.lastAutoTable?.finalY||200;
    doc.setFontSize(11); doc.setFont("helvetica","bold"); doc.text("Notes",14,tableEndY+12);
    doc.setFont("helvetica","normal"); doc.setFontSize(10);
    doc.text(doc.splitTextToSize(notes||"",180),14,tableEndY+20);
      ...
      <div style={{background:TM.dark,padding:"0 32px",borderBottom:`3px solid ${TM.tanDeep}`}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",height:70}}>
          ...
          <div style={{display:"flex",gap:10}}>
            <button style={{...buttonStyle,background:"transparent",color:TM.tan,borderColor:TM.tanDeep,fontSize:12,fontFamily:"Arial, sans-serif"}} onClick={resetDefaults}><RefreshCcw size={13} style={{marginRight:6,verticalAlign:"middle"}}/>Reset</button>
            <button style={{...buttonStyle,background:"transparent",color:TM.tan,borderColor:TM.tanDeep,fontSize:12,fontFamily:"Arial, sans-serif"}} onClick={()=>setQuotesOpen(true)}><List size={13} style={{marginRight:6,verticalAlign:"middle"}}/>Quotes</button>
            <button style={{...buttonStyle,background:"transparent",color:TM.tan,borderColor:TM.tanDeep,fontSize:12,fontFamily:"Arial, sans-serif"}} onClick={()=>setOrdersOpen(true)}><ClipboardList size={13} style={{marginRight:6,verticalAlign:"middle"}}/>Orders</button>
          </div>
        </div>
      </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:24}}>
            ...
            <div style={{display:"grid",gap:20}}>
              <div style={cardStyle}>
                ...
                <div style={{marginBottom:16}}>
                  <div style={{fontSize:11,fontWeight:700,color:TM.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>
                  ...
                  <button onClick={addLineItem} style={{...buttonStyle,width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:TM.tanBg,borderStyle:"dashed",color:TM.tanDeep,borderColor:TM.tanDeep,fontFamily:"Arial, sans-serif"}}>
                    <Plus size={15}/> Add Another Item
                  </button>
                </div>

                {/* Shared fees */}
                <div style={{paddingTop:16,borderTop:`1px solid ${TM.border}`}}>
                  <div style={{fontSize:11,fontWeight:700,color:TM.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>Shared Fees & Settings</div>
            ...
            <div style={{display:"grid",gap:20,alignSelf:"start"}}>
              <div style={cardStyle}>
                ...
                {/* Per-item breakdown */}
                {lineItems.filter(i=>i._qty>0).map((item,idx)=>(
                  <div key={item.id} style={{background:TM.tanBg,borderRadius:8,padding:"8px 12px",marginBottom:8,border:`1px solid ${TM.tanBorder}`,fontSize:12}}>
                    <div style={{fontWeight:700,color:TM.dark,marginBottom:4}}>Item {idx+1}: {item._garmentLabel}</div>
                    <div style={{display:"flex",justifyContent:"space-between",color:TM.muted}}><span>{item._qty} pcs · {currency(item._garmentCostEach)}/ea garment · {currency(item._decorationCostEach)}/ea DTF</span></div>
                    <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{color:TM.muted}}>Line subtotal</span><span style={{fontWeight:700,color:TM.dark}}>{currency((item._garmentCostEach+item._decorationCostEach)*item._qty)}</span></div>
                  </div>
                ))}

                <SummaryRow label="Total quantity" value={String(totalQty)}/>
                <SummaryRow label="Garment subtotal" value={currency(totalGarmentSubtotal)}/>
                <SummaryRow label="DTF subtotal" value={currency(totalDecorationSubtotal)}/>
                ...
                {isManual&&<SummaryRow label="Manual pricing active" value={`${totalQty} × ${currency(safeNum(manualPriceEach))}`} accent/>}
              ...
              <div style={{...cardStyle,border:`2px solid ${TM.dark}`}}>
                ...
                <div style={{background:TM.tanBg,borderRadius:10,padding:14,fontSize:13,marginBottom:14,border:`1px solid ${TM.tanBorder}`}}>
                  {lineItems.filter(i=>i._qty>0).map((item,idx)=>(
                    <div key={item.id} style={{padding:"6px 0",borderBottom:`1px solid ${TM.tanBorder}`,display:"flex",justifyContent:"space-between"}}>
                      <span style={{color:TM.muted}}>Item {idx+1}</span>
                      <span style={{fontWeight:600,color:TM.dark,textAlign:"right",maxWidth:"70%"}}>{item._garmentLabel} <span style={{color:TM.muted,fontWeight:400}}>({item._qty} pcs)</span></span>
                    </div>
                  ))}
                  <div style={{padding:"6px 0",display:"flex",justifyContent:"space-between"}}>
                    <span style={{color:TM.muted}}>Total Quantity</span>
                    <span style={{fontWeight:700,color:TM.dark}}>{totalQty}</span>
                  </div>
                </div>
